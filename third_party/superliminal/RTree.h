...
#define RTREE_DONT_USE_MEMPOOLS  // This version does not contain a fixed memory
                                 ...
                                 // implement one.
#define RTREE_USE_SPHERICAL_VOLUME  // Better split classification, may be
...
class RTree {
 ...
 public:
  ...
  /// Function will reset, not append to array. \param a_resultCallback Callback
  ...
  class Iterator {
   ...
   private:
    ...
    friend RTree;  // Allow hiding of non-public functions while allowing
...
bool RTREE_QUAL::InsertRectRec(Rect* a_rect,
  ...
  if (a_node->m_level > a_level) {
    ...
    if (!InsertRectRec(a_rect, a_id, a_node->m_branch[index].m_child,
                       &otherNode, a_level)) {
      // Child was not split
  ...
  } else if (a_node->m_level == a_level)  // Have reached level for insertion.
