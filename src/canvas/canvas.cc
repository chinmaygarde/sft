...
std::shared_ptr<CanvasContext> CanvasContextCreate() {
  return std::make_unique<CanvasContext>();
}

Canvas::Canvas(std::shared_ptr<CanvasContext> context)
    : context_(std::move(context)) {}

void Canvas::DrawRect(Rasterizer& rasterizer, Rect rect, const Paint& paint) {
  ...
  auto vertex_buffer = buffer->Emplace(std::vector<VD>{
      ...
      VD{.position = rect.origin + glm::vec2{rect.size.width, 0.0},
         .uv = {1, 0}},
      VD{.position = rect.origin + glm::vec2{rect.size.width, rect.size.height},
         .uv = {1, 1}},
  ...
  pipeline->color_desc = paint.color_desc.value_or(ColorAttachmentDescriptor{});

  pipeline->depth_desc = paint.depth_desc.value_or(DepthAttachmentDescriptor{});
...
void Canvas::Translate(glm::vec2 tx) {
  ctm_ = glm::translate(ctm_, glm::vec3{tx, 0.0});
}

void Canvas::Concat(const glm::mat4& xform) {
  ctm_ *= xform;
}
